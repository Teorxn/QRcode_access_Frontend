...
import { listAccessHistory, listAreas, exportAccessPdf, exportAccessExcel } from "@/lib/repositories";
...
export default function Record() {
  ...
  const [selectedStatus, setSelectedStatus] = useState<
    "all" | "EXITOSO" | "FALLIDO"
  >("all");
  const [loadingExport, setLoadingExport] = useState<"pdf" | "excel" | null>(null);
  ...
  const handleExport = async (format: "pdf" | "excel"): Promise<void> => {
    ...
    try {
      ...
      const blob = format === "pdf" ? await exportAccessPdf(filters) : await exportAccessExcel(filters);
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `accesos_${new Date().toISOString().slice(0, 10)}.${format === "excel" ? "xlsx" : format}`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
    } catch {
      alert("Error al exportar. Verifique la conexión con el servidor.");
    } finally {
      ...
      <div className="flex justify-between items-center mb-4 px-2">
        ...
        <div className="flex gap-3">
          <button
            ...
            className="flex items-center gap-2 bg-white border border-[#e2e8f0] text-[#1b254b] font-medium py-2 px-4 rounded-lg hover:bg-gray-50 transition-colors text-sm disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loadingExport === "pdf" ? (
          ...
          <button
            ...
            disabled={loadingExport !== null}
            className="flex items-center gap-2 bg-[#05cd99] hover:bg-[#04b688] text-white font-medium py-2 px-4 rounded-lg transition-colors text-sm disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loadingExport === "excel" ? (
              <svg className="animate-spin" width="16" height="16" viewBox="0 0 24 24" fill="none">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
